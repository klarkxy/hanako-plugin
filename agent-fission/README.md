...
- 新角色创建默认走 `contentMode=replace`：先在 `hanako` / `butter` / `ming` / `kong` 中选最贴近的 `yuan`
- 基于所选 `yuan` 的默认稿做“按段改写”，不要把人设直接拼接到旧提示词末尾
- `overlay` 仅用于小补丁（给现有模板加几条短约束），不是完整角色落地模式
- 注意 tool 默认是 `replace`，只有明确需要小补丁时才传 `contentMode: "overlay"`
- `identity` 和 `ishiki` 聚焦于这个 agent 的独特性，避免语义重复和同义规则重复
...
- 新角色建议：先选 `yuan`，再基于默认稿重写后用 `contentMode=replace` 写入。
- `contentMode=overlay` 只用于小补丁追加，不建议用于完整角色创建。