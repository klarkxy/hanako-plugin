...
| 🧬 身份叠加 | 保留 Hanako 模板 + 叠一层 `identity.md` |
| 🧠 意识叠加 | 保留 Hanako 模板 + 叠一层 `ishiki.md` |
...
- 这些补充会叠加在 Hanako 自带默认模板上
- 同人角色、OC 人设、文学型 agent 都行——用户想要什么调性就做什么调性

头像查找流程：

- 当需要头像但用户没有提供时，快速搜 3-5 张候选图展示给用户挑
- 不要花时间精挑细选——快速展示，让用户决定
- 用户不满意就继续搜，或让用户自己提供图片
- 用户选定后，让用户自行裁切，或使用图片编辑工具处理
